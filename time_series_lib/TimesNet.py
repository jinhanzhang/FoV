...
class TimesNet(nn.Module):
    ...
    def __init__(self, task_name = 'long_term_forecast',seq_len = 128,label_len=128, pred_len = 0, d_model = 256,\
                 c_out=256, embed = 'timeF', freq='h', dropout = 0.1, factor=3, n_heads =8 ,d_ff=256, \
                 activation = 'gelu',norm=False,\
                 e_layers =2,d_layers=1,enc_in = 180, dec_in = 180,top_k = 3, num_kernels = 6 ):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec,norm=False,debug=False):
        ...
        enc_out = self.predict_linear(enc_out.permute(0, 2, 1)).permute(
            0, 2, 1)  # align temporal dimension, it is ok in multi, just op in time
        # TimesNet
        if debug:
            print ('predict_linear', enc_out.shape)
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask,norm=False):
        if norm:
        # Normalization from Non-stationary Transformer
