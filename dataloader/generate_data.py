...
def createSequence(files, history_time=10, target_time=10, step=15, window_size=60,denoise_flag=False):
    x_list = []
    y_list = []
...
def generate_data(path, save_path, hist_time, pred_time,frame_rate,denoise_flag=False):
    ...
    other_files = glob.glob(path + '/intersection*.txt')+ glob.glob(path + '/original*.txt')+glob.glob(path + '/output*.txt')
    train_files = list(filter(lambda i: i not in test_files and (i not in val_files and i not in other_files), train_files))
    x_train,y_train, mean_std_train = createSequence(train_files,hist_time,pred_time,window_size=frame_rate,denoise_flag=denoise_flag)
    x_val,y_val, mean_std_val = createSequence(val_files,hist_time,pred_time,window_size=frame_rate,denoise_flag=denoise_flag)
    x_test,y_test, mean_std_test = createSequence(test_files,hist_time,pred_time,window_size=frame_rate,denoise_flag=denoise_flag)
...
def denoise(df, window_length=20, polyorder=3, deriv=0, delta=1.0, axis=-2, mode='interp'):
    ...
    """
    data = df.to_numpy()
    denoised_data = avgol_filter(data, window_length, polyorder)
    plt.figure()
